...
} from "./openingTickOffset.js";
import { OpeningAutomationInput } from "./openingAutomation.js";
...
var settings = {
  ...
  openingAutomationTickOffset: "-9.5",
  openingAutomationTickOffsetUnit: OFFSET_UNIT_VERSION,
...
const settingsManager = new (function () {
  ...
  settingsStructure.forEach((item) => {
    ...
    if (item.min !== undefined) element.min = item.min;
    if (item.max !== undefined) element.max = item.max;
    if (item.step !== undefined) element.step = item.step;
...
if (localStorage.getItem("fx_settings") !== null) {
  const savedSettings = JSON.parse(localStorage.getItem("fx_settings") || "{}");
  const migrateSavedOpeningTickOffset = shouldMigrateOpeningTickOffset(savedSettings);
  __fx.settings = settings = {
    ...settings,
    ...savedSettings,
  };
  if (migrateSavedOpeningTickOffset) {
